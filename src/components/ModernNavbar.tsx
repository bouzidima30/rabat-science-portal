            ...
            {menuItems.map(item => <div key={item.name} className="relative" onMouseEnter={() => item.hasDropdown && setHoveredMenu(item.name)} onMouseLeave={() => setHoveredMenu(null)}>
                {item.hasDropdown ? <>
                    ...
                    {hoveredMenu === item.name && <div className="absolute top-full left-0 w-72 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 z-50 overflow-hidden">
                        {item.dropdownItems?.map(dropdownItem => <Link key={dropdownItem.name} to={dropdownItem.path} className="block px-6 py-4 text-sm text-gray-700 dark:text-gray-200 hover:bg-blue-50 dark:hover:bg-gray-700 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200 border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                            {dropdownItem.name}
                          </Link>)}
                      </div>}
                  </> : <Link to={item.path}>
