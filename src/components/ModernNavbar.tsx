  ...
  return <nav className="bg-white dark:bg-gray-900 shadow-lg border-b-2 border-blue-600 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-20">
          ...
          {/* Mobile Menu Button */}
          <Button variant="ghost" size="icon" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden text-gray-700 dark:text-gray-200 hover:text-blue-600 relative">
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && <div className="lg:hidden fixed inset-0 z-[55] bg-black/50 backdrop-blur-sm" onClick={() => setIsMobileMenuOpen(false)} />}

      {/* Mobile Menu */}
      <div className={`lg:hidden fixed top-0 right-0 h-full w-full max-w-sm bg-white dark:bg-gray-900 shadow-2xl z-[55] transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full">
          {/* Mobile Header */}
          <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Menu</h2>
            <Button variant="ghost" size="icon" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-700 dark:text-gray-200">
              <X className="h-6 w-6" />
            </Button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto py-4">
            {menuItems.map(item => <div key={item.name} className="border-b border-gray-100 dark:border-gray-800 last:border-b-0">
                {item.hasDropdown ? <div>
                    <button onClick={() => toggleMobileSubmenu(item.name)} className="w-full flex items-center justify-between p-4 text-left hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      <span className="text-lg font-medium text-gray-900 dark:text-white">
                        {item.name}
                      </span>
                      <ChevronRight className={`h-5 w-5 text-gray-500 transition-transform duration-200 ${expandedMobileMenu === item.name ? 'rotate-90' : ''}`} />
                    </button>
                    
                    {expandedMobileMenu === item.name && <div className="bg-gray-50 dark:bg-gray-800">
                        {item.dropdownItems?.map(dropdownItem => <button key={dropdownItem.name} onClick={() => handleMobileNavigation(dropdownItem.path)} className="w-full text-left p-4 pl-8 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-blue-600 dark:hover:text-blue-400 transition-colors border-t border-gray-200 dark:border-gray-700 first:border-t-0">
                            {dropdownItem.name}
                          </button>)}
                      </div>}
                  </div> : <button onClick={() => handleMobileNavigation(item.path)} className="w-full text-left p-4 text-lg font-medium text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                    {item.name}
                  </button>}
              </div>)}
          </div>
