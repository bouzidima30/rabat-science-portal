...
const TopBar = () => {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    if (typeof window !== 'undefined') {
      return document.documentElement.classList.contains('dark') || localStorage.getItem('theme') === 'dark' || !localStorage.getItem('theme') && window.matchMedia('(prefers-color-scheme: dark)').matches;
    }
    return false;
  });
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            {user ? <DropdownMenu>
                ...
                <Link to="/login">
                  <Button variant="outline" size="sm" className="text-white border-white/30 hover:bg-white/10 bg-[#016be5]">
                    Connexion
                  </Button>
                </Link>
                <Link to="/register">
