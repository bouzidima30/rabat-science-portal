...
const AdminPages = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('pages').select('*').order('created_at', {
        ascending: false
      });
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('page-images').upload(filePath, file);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadPromises = Array.from(files).map(async file => {
        ...
        const {
          error: uploadError
        } = await supabase.storage.from('page-documents').upload(filePath, file);
        if (uploadError) throw uploadError;
        const {
          data
        } = supabase.storage.from('page-documents').getPublicUrl(filePath);
    ...
    <div className="p-4 sm:p-6 lg:p-8 bg-muted/30 min-h-screen">
      ...
      <div className="space-y-6">
        {paginatedPages.map((page) => (
          <Card key={page.id} className="border-0 shadow-lg hover:shadow-xl transition-all duration-200 bg-card">
            <CardContent className="p-6">
