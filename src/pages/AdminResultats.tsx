...
import { Folder, File, Trash2, Archive, Loader2, Copy, Award } from "lucide-react";
...
const CATEGORIES = [
  { key: "resultats_automne", label: "Session d'Automne" },
  { key: "resultats_printemps", label: "Session de Printemps" },
];

const AdminResultats = () => {
  ...
  const deleteItemMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-resultats"] });
    ...
    try {
      ...
      for (const [relativePath, zipEntry] of sorted) {
        ...
        if (zipEntry.dir) {
          ...
          const storagePath = `resultats/${storageName}`;
    ...
    return (
      <div className="space-y-4">
        <div className="flex justify-end gap-2">
          ...
          <Button size="sm" asChild disabled={uploading} className="bg-emerald-600 hover:bg-emerald-700 shadow-lg">
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-8">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Résultats des Examens</h1>
            ...
            <Card key={cat.key} className={`border-0 shadow-lg bg-gradient-to-r ${theme.grad}`}>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <Award className={`h-6 w-6 ${theme.color}`} />
                </div>
              </CardContent>
            </Card>
          );
